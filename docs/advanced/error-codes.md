...
> **Retired**: images since 2026-07-23 use the status-file boot handshake instead of the marker heartbeat and report [KL-DMN-03](#kl-dmn-03) / [KL-DMN-04](#kl-dmn-04). This code only appears on older images - the diagnosis below still applies there.
...
> **Retired**: images since 2026-07-23 detect a missing mount as "ready but no readable VPK" and report [KL-DMN-04](#kl-dmn-04). This code only appears on older images - the diagnosis below still applies there.
...
### KL-DMN-03 - Daemon reported a failed push

**Symptom**: `[KL-DMN-03] Daemon reported a failed push - falling back to SteamCMD`

**Meaning**: The daemon's push worker for this server wrote `state=failed` into `egg/.daemon-status` (rsync/link failure, permissions, disk full, a push worker crash, or a central update lock held for over an hour).
...
**Fix**:
1. Resolve the cause from the daemon logs (disk space, permissions).
2. Restart the server - the daemon re-verifies and re-pushes on the next boot.

---

<a id="kl-dmn-04"></a>

### KL-DMN-04 - Daemon ready but no readable VPK

**Symptom**: `[KL-DMN-04] Daemon reported ready but no readable VPK found - falling back to SteamCMD`

**Meaning**: The status file says `done`, but not a single VPK resolves inside the container. In symlink mode this is almost always a missing `/tmp/cs2-shared` bind mount (kernel < 5.2 or missing `python3` on the host), leaving dangling symlinks.
...
**Fix**:
1. Ensure kernel ≥ 5.2 and `python3` on the host, or switch `VPK_PUSH_METHOD` to `hardlink` / `copy`.
2. Restart the daemon, then the server: `sudo systemctl restart cs2-vpk-daemon`
